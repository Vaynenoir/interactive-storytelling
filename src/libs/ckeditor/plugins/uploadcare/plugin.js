(function e(t, n, r) {
                ...
                var dialog = uploadcare.openDialog(file, settings).done(function(selected) {
                    ...
                    uc.jQuery.when.apply(null, files).done(function() {
                        ...
                        $("input[name=light-trigger]").on("click", function() {
                            ...
                            var AllGalleries = $(htmlDoc).find("div.slick_gallery");

                            if ($(gallery).parents().hasClass(".slick_gallery")) {
                                $(gallery).parents().remove();
                            }

                            console.log(AllGalleries);

                            $.each(AllGalleries, function(i, el) {
                            ...
                            var AllimgElements = $(htmlDoc).find("img");
                            $.each(AllimgElements, function(i, el) {
                                if ($(el).parent().parents().hasClass(".slick_gallery")) {
                                    $(el).parent().parents().remove();
                                }
                            });
                            ...
                            // var docBody = $(htmlDoc).find("body");
                            // console.log(docBody);
                            // console.log("KEK        " + htmlDoc.innerHTML);
                            var docBody = htmlDoc.body.innerHTML;
                            // console.log(htmlDoc.body.innerHTML);


                            // CKEDITOR.on("instanceReady", function (event) {
                            //     editor = event.editor;
                            //     editor.setData(docBody);
                            //     // YOUR OTHER CODE 
                            // });

                            editor.setData("");
                            editor.insertHtml(docBody);
                            // console.log($(htmlDoc).find("div.slick_gallery"));
                        });
    ...
    }, { "../tools/search-selected-element": 8 }],
    ...
    }, { "./commands/show-uploadcare-dialog": 1, "./styles/resize": 4, "./tools/clear-toolbar": 5, "./tools/find-one": 6, "./tools/get-body": 7 }],
